...
* Predictions should not depend on which observations are present in `new_data`. This occasionally happens when using `stats::poly()` and `splines::ns()`.

* The prediction code should have a well documented default behavior when novel factor levels appear in the test set for factor predictors, or should throw an informative error.
...
Specific cases:
   ...
   * If interval estimates are produced (e.g. prediction/confidence/credible), the column names should be `.pred_lower` and `.pred_upper`. If a standard error is produced, it should be named `.pred_std_error`. 
