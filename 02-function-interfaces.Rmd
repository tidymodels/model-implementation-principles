...
* If possible, check to see if the arguments passed to `...` are real arguments to the function(s) receiving the `...`.  

<div id="standardized-back"></div>

* Common arguments should use standardized names. [{note}](#standardized)

* If the call object is saved, it should be checked for size. In cases where the function is invoked by `do.call("foo", bar)`, the data set may be embedded in the argument list `bar`. 
